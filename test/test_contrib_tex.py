...
class RecordedTest(tools_for_test.TemporaryWorkingDirectoryTestCase):
...
class LatexTest(tools_for_test.TemporaryWorkingDirectoryTestCase):
    ...
    def test_fails_for_lf_in_cwd(self):
        os.mkdir('out\nput')
        with tools_for_test.DirectoryChanger('out\nput'):
            os.mkdir('.dlbroot')
            open('report.tex', 'xb').close()
