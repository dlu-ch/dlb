...
class CreationTest(testenv.TemporaryDirectoryTestCase):

    def test_file_exists_after_construction(self):
        try:
            os.mkdir('a:b')
        except OSError:
            # POSIX does not require the support of ':' in a file name
            raise unittest.SkipTest("requires filesystem that supports ':' in filename") from None
...
class CreationWithPermissionProblemTest(testenv.TemporaryDirectoryWithChmodTestCase):
...
class DeclareFsobjectInputAsModifiedTest(testenv.TemporaryDirectoryTestCase):
...
class ReplaceFsInputsTest(testenv.TemporaryDirectoryTestCase):
...
class ReplaceAndGetRedoStateTest(testenv.TemporaryDirectoryTestCase):
...
class CommitTest(testenv.TemporaryDirectoryTestCase):
...
class CleanupTest(testenv.TemporaryDirectoryTestCase):
...
class RunSummaryTest(testenv.TemporaryDirectoryTestCase):
